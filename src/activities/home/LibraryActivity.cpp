...
void LibraryActivity::onEnter() {
  ...
  // New EPUBs trigger the full-screen indexing progress view. The callback
  // fires only when there's work, so a warm cache never shows it. Each repaint
  // is a blocking ~400ms e-ink refresh, so throttle to ~5% buckets (always
  // painting the first book and 100%) — ~20 refreshes over a cold index.
  int lastBucket = -1;
  LIBRARY_INDEX.refreshFromSdCard([this, &lastBucket](int done, int total, const char* label) {
    if (total <= 0) return;
...
void LibraryActivity::renderIndexingProgress(int done, int total, const char* label) {
  renderer.clearScreen();
  ...
  {
    const Rect r = flex::center(rows[3], renderer.getTextWidth(bodyFont, countBuf), countH);
  ...
  if (label != nullptr && label[0] != '\0') {
    ...
    renderer.drawText(capFont, r2.x, r2.y, truncatedLabel.c_str(), true, EpdFontFamily::ITALIC);
  }
  ...
  // One-time expectation-setter, ~1.5 button-hint heights above the screen bottom.
  {
    ...
    const Rect r = flex::center(noteRow, renderer.getTextWidth(capFont, note, EpdFontFamily::ITALIC), labelH);
